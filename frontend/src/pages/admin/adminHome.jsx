    ...
    <div className="flex">
      ...
      >
        ...
        <main className="p-10 pt-24 bg-gray-200 min-h-screen overflow-y-scroll overflow-x-scroll">
          ...
          <div className="bg-white p-8 text-black rounded-lg shadow-md w-full max-w-4xl mt-5">
            ...
            <div>
              {table.getRowModel().rows.length > 0 ? (
                <>
                  {table.getHeaderGroups().map((headerGroup) => (
                    <div key={headerGroup.id} className="flex">
                      {headerGroup.headers.map((header) => (
                        <div
                          key={header.id}
                          className="flex-1 font-bold text-left text-white bg-[#2b193d] border border-gray-600 p-2"
                        >
                  ...
                  {table.getRowModel().rows.map((row) => (
                    <div key={row.id} className="flex">
                      {row.getVisibleCells().map((cell) => (
                        <div
                          key={cell.id}
                          className="flex-1 text-left border border-gray-600 p-2"
                        >
                          {cell.getValue()}
                        </div>
                      ))}
                  ...
                  <button className="border border-gray-600 text-15" onClick={table.getState().pagination.previousPage} isDisabled={!table.getState().pagination.hasPreviousPage}>{"<"}</button>
                  <button className="border border-gray-600 text-15" onClick={table.getState().pagination.nextPage} isDisabled={!table.getState().pagination.hasNextPage}>{">"}</button>
                </>
              ) : (
                <div className="flex">
